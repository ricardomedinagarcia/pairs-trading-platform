...
def _approx_eg_pvalue(adf_stat: float) -> float:
    """Compute p-value for Engle-Granger ADF statistic using MacKinnon (1996).

    Delegates to statsmodels' implementation of MacKinnon's response surface,
    which gives continuous p-values down to ~10^-12.

    Note: this is not a statsmodels-replacement -- our Engle-Granger TEST
    implementation is hand-built and validated separately. We use statsmodels
    here only for the p-value lookup function (MacKinnon coefficients), which
    would otherwise require ~200 lines of polynomial coefficients to inline.
    """
    from statsmodels.tsa.adfvalues import mackinnonp

    # mackinnonp args: (test_stat, regression, N=k where k is num vars in cointegration)
    # regression='c' = constant, no trend; N=2 = bivariate cointegration
    return float(mackinnonp(adf_stat, regression="c", N=2))
